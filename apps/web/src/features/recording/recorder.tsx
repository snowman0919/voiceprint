  ...
  return (
    <section className="recorder" aria-labelledby="recording-heading">
      ...
      {analysis && <section aria-labelledby="result-heading" className="result">
        ...
        <dl className="quality"><div><dt>F0 중앙값</dt><dd>{Math.round(analysis.acousticFeatures.f0MedianHz ?? 0)}Hz</dd></div><div><dt>F0 5–95%</dt><dd>{Math.round(analysis.acousticFeatures.f0P05Hz ?? 0)}–{Math.round(analysis.acousticFeatures.f0P95Hz ?? 0)}Hz</dd></div><div><dt>스펙트럼 중심</dt><dd>{Math.round(analysis.acousticFeatures.spectralCentroidHz ?? 0)}Hz</dd></div>{analysis.acousticFeatures.hnrDb !== undefined && <div><dt>HNR</dt><dd>{analysis.acousticFeatures.hnrDb.toFixed(1)}dB</dd></div>}<div><dt>입력 품질</dt><dd>{analysis.quality.score}</dd></div></dl>
        <h3>연습 제안</h3>
