...
export function ModelStatus() {
  ...
  const download = useCallback(async (active: ModelEntry) => {
    abort.current?.abort();
    const controller = new AbortController();
    abort.current = controller;
    setStatus("downloading"); setReceived(0); setError(undefined);
    try {
      ...
      if (controller.signal.aborted) { setStatus("unavailable"); return; }
  ...
  if (status === "unavailable") return <section aria-label="분석 모델 상태" className="model-status"><strong>분석 모델 다운로드 가능</strong><p>{((model?.size ?? 0) / 1_000_000).toFixed(1)} MB · 이 기기에만 저장합니다.</p><button onClick={() => model && void download(model)} type="button">분석 모델 받기</button></section>;
  ...
  if (status === "downloading") return <section aria-label="분석 모델 상태" className="model-status"><strong>분석 모델 준비 중</strong><p>{(received / 1_000_000).toFixed(1)} MB / {((model?.size ?? 0) / 1_000_000).toFixed(1)} MB</p><button onClick={() => abort.current?.abort()} type="button">취소</button></section>;
  return <section aria-label="분석 모델 상태" className="model-status"><strong>모델 준비 실패</strong><p>{error}</p>{model && <button onClick={() => void download(model)} type="button">다시 시도</button>}</section>;
}
