...
import { summarizeF0, summarizeFormants, summarizeVoiceQuality } from "../dsp";
...
describe("summarizeVoiceQuality", () => {
  it("reports zero frame-to-frame variation for a stable voiced signal", () => {
    expect(summarizeVoiceQuality([200, 200, 200], [0.2, 0.2, 0.2])).toEqual({ jitterPercent: 0, shimmerPercent: 0 });
  });
});
