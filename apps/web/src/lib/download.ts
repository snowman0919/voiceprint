...
export function downloadSummaryPng(result: LocalAnalysis) {
  ...
  const rows = [
    ["F0 중앙값", `${Math.round(result.acousticFeatures.f0MedianHz ?? 0)}Hz`],
    ["스펙트럼 중심", `${Math.round(result.acousticFeatures.spectralCentroidHz ?? 0)}Hz`],
    ["유성음", `${Math.round(result.quality.voicedRatio * 100)}%`],
    ["입력 품질", `${result.quality.score}`],
  ];
  rows.forEach(([label, value], index) => {
    const y = 170 + index * 60;
    context.fillText(label, 48, y);
    context.fillText(value, 440, y);
  });
  context.font = "400 14px Arial";
  context.fillStyle = "#52606d";
  context.fillText(`생성일 ${result.createdAt.slice(0, 10)} · DSP ${result.dspVersion}`, 48, 414);
  canvas.toBlob((blob) => {
    if (blob) save(blob, "voiceprint-summary.png");
  }, "image/png");
}
