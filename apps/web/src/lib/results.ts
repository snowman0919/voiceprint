...
export const practiceGoals = ["clarity", "stability", "brightness", "softness", "calm", "lightness", "intonation", "relaxation"] as const;
...
export function recommendations(quality: AudioQuality, dsp: DspSummary, goal: PracticeGoal) {
  ...
  if (goal === "stability" && (dsp.f0Stability ?? 100) < 75) result.push("목에 힘을 주어 높이를 고정하지 말고, 편안한 음역에서 일정한 호흡으로 문장을 반복하세요.");
  if (goal === "brightness" && (dsp.spectralCentroidHz ?? 0) < 1_500) result.push("음을 세게 밀기보다 입 앞쪽에서 울리는 느낌으로 명료한 모음을 유지해 보세요.");
  ...
  if (goal === "intonation" && (dsp.f0SemitoneRange ?? 0) < 2) result.push("핵심 단어에서만 높이를 조금 올리고, 문장 끝의 높이를 의도적으로 정리해 보세요.");
...
export function createLocalAnalysis(input: LocalAnalysis["input"], quality: AudioQuality, dsp: DspSummary, appVersion: string, dspVersion: string, practiceGoal: PracticeGoal = "clarity"): LocalAnalysis {
  return {
    schemaVersion: 1,
