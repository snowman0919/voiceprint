...
For the personal-result service, the browser sends only a scalar result record after local analysis: a random recovery ID, timestamp, displayed impression summary, scalar acoustic/quality values, and each metric group's provenance (`direct_acoustic_measurement` or `deterministic_derived_metric` for the currently available report). It does not send an audio file or frame array. The result service stores this record in SQLite for 365 days, purges expired records on service activity, and permits deletion when the browser presents the matching recovery ID.
