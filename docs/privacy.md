...
Share links use the URL fragment. A fragment is not part of an HTTP request to the host. The payload excludes audio, trajectories, embeddings, identifiers, and undeployed model scores; it contains only the displayed acoustic summary. It can be modified by the person holding the link and is not an official certification.
